// procedural_path_tracer.cu
// Compile: nvcc procedural_path_tracer.cu -O2 -arch=sm_86 -o procedural_path_tracer
...
static float prompt_float(const std::string& message, float default_value) {
    for (;;) {
        std::cout << "\n" << message;
        std::string line;
        if (!std::getline(std::cin, line) || line.empty()) {
        ...
        std::cout << "Please enter a number or press Enter to use the default value.\n";
    }
}
...
// ---------- host main ----------
int main(){
    std::cout << "Procedural Path Tracer (CUDA)\n";
    std::cout << "This scene is GPU-only and is the heaviest renderer in the project.\n";
    std::cout << "Recommended first test: 1280x720, 32-64 samples, 6 bounces, fog around 0.02.\n";
    ...
    float fog_density = prompt_float("Fog density [Enter=0.02, 0.0 = none]: ", 0.02f);

    std::cout << "\nRender setup:\n";
    std::cout << "  Resolution: " << width << "x" << height << "\n";
    std::cout << "  Samples per pixel: " << samples << "\n";
    std::cout << "  Max bounces: " << maxDepth << "\n";
    std::cout << "  Fog density: " << fog_density << "\n";
    ...
    std::string name = make_output_name("procedural_path_tracer", "gpu", render_stamp);
